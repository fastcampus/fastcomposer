<template>
  <pane :title="`preview`">
    <main :style="{ zoom }">
      <div class="preview__box" v-for="(layer, layerIndex) in layers" :key="'layer-' + layerIndex">
        <layer-preview :layer="layer" @selectPreview="$emit('select', layer)" :active="selectedLayer.id === layer.id">
        </layer-preview>
